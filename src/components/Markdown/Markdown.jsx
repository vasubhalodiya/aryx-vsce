...
const IMG_PLACEHOLDER_SVG = `<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>`;

const renderer = new Renderer();

renderer.code = ({ text, lang }) => {
  const language = (lang || 'plaintext').toLowerCase();
  return `<div class="md-code-block"><div class="md-code-header"><span class="md-code-lang">${escHtml(language)}</span><button class="md-copy-btn" data-code="${encodeURIComponent(text)}" title="Copy">${COPY_SVG}</button></div><pre><code>${escHtml(text)}</code></pre></div>`;
};

renderer.image = ({ href, text }) => {
  return `<span class="md-img-wrap"><img src="${escHtml(href || '')}" alt="${escHtml(text || '')}" class="md-img" /><span class="md-img-broken" style="display:none">${IMG_PLACEHOLDER_SVG}</span></span>`;
};
...
export default function Markdown({ text }) {
  ...
  const html = useMemo(() => {
    ...
    h = h.replace(/<table>/g, '<div class="md-table-wrap"><table>').replace(/<\/table>/g, '</table></div>');
    return h;
  }, [text]);
  ...
  useEffect(() => {
    ...
    el.querySelectorAll('.md-img').forEach((img) => {
  ...
  return <div className="bubble md-bubble" ref={ref} dangerouslySetInnerHTML={{ __html: html }} />;
}
